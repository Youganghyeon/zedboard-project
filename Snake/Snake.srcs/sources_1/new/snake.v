module display(
	// ?’·?Ÿ•?topŸº?
    input clock, // 148.5MHZ£¨ÈƒÈÕ?ıÛ1920x1080@60HzÓ‹VGA„·?
    input [5:0] apple_x,
    input [5:0] apple_y,
	...
	// snake_x[0]£∫?Ó‹?Ò¶?  snake_y[0]:?Ó‹?Ò¶?
    reg [5:0] snake_x [31:0];
	reg [5:0] snake_y [31:0];
	
	reg [11:0] count;
	
	// ?ÓÒx£¨yÒ¶?£¨0<=x<=47,0<=y<=26
	reg [5:0] current_x;
	reg [5:0] current_y;
	...
	// ÈƒÈÕŸº????Ï£??? [5:0] snake_x/y [31:0]
	// ?Õ≈Ìª https://stackoverflow.com/questions/16369698/how-to-pass-array-structure-between-two-verilog-modules
	integer i;
    ...
    begin
    	if (game_status==LAUNCHING)
    		begin
    			// ‹‚„ÛÈ©?„∆Ó‹?1920x1080·∂‚ÕROM‰¢”¯ı±???ROM‰¢”¯£¨Û˙Èƒ??ROMÍÛﬁ¡??£¨
    			// Î¶ÈÕ???Ò˛ÍÛÒÈ?›ª›¬ÍÛ?‡§£¨Õ∫˜Í?¯∏ÓÆÓÚ€ØÒÈ?? ¶£®ÓÆÓÚ?1320x770£©°£
				if (x_counter>=300 && x_counter<1620 && y_counter>=180 && y_counter<950)
				begin
					// ?ÈÕ˙∞?ROM?ˆ¢Ê≈?£¨‹‚Ï—Û˙ÈƒÓ‹€∞€ˆ?Â∞?£®˘ªÿ¸Ó‹185£¨945£¨295‘¥„¿?ıÛ?Ó‹£©£¨œ¥Ã⁄˚øÓ‹€∞€ˆ°£
					address<=(y_counter-180)*1320+x_counter-300-295;
