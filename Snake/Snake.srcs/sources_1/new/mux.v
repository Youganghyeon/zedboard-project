...
module snake(
	...
	output reg [31:0] snake_piece_is_display, // Íóğ¤Şï??
    ...
    reg [25:0] count;
    reg [25:0] count_two;
    reg [31:0] snake_piece_is_display_origin; // ğí???îÜ??£¬éÄéÍŞİØÌ??
	...
	// snake_x[0]£º?îÜ?ñ¦?  snake_y[0]:?îÜ?ñ¦?
    ...
    begin
    	...
    	begin
    		snake_piece_is_display_origin<=snake_piece_is_display; // ğí???îÜ??£¬éÄéÍŞİØÌ??
    		
    		if (snake_x[0]==0 || snake_x[0]==47 || snake_y[0]==0 || snake_y[0]==26) hit_wall<=1;
    		else hit_wall<=0; // ãÀÜúÓÕ?
    		...
    		hit_itself<=1;
    		else hit_itself<=0; // ãÀÜúÓÕí»Ğù
    		
    		if (snake_x[0]==apple_x && snake_y[0]==apple_y) get_apple<=1;
    		else get_apple<=0; // ãÀÜúıŞÓğ?Íı
    		
    		if (get_apple==1 ) 
    		begin
    		snake_piece_is_display<=2*snake_piece_is_display+1; // ñòÊ¥??£¨÷êsnake_piece_is_displayõÌı¨ìé?0?à÷1£©
    		get_apple<=0;
    		end
    		
    		current_direction<=next_direction; // ÌÚãæÛ°ú¾
    		
    		if (count<8000000*(2+slow_down)) // Íóğ¤áÜÓø
    			count<=count+1;
    		else
    		begin
				count<=0;
