...
module score(
    ...
    assign real_enable = (get_apple==1) && (game_status==PLAYING); // ?counterŞÅÒö
    assign real_reset = (reset==1) | (game_status==INITIALIZING); // ?counter?Íö
    
    wire [3:0] score_a,score_b,score_c,score_d; // İÂ??ô¶êÈ¡¢ÛİêÈ¡¢ä¨êÈ¡¢?êÈ?í®îÜ4êÈBDC?
    wire [3:0] threshold; // éÄéÍ?×â?êÈ
    ...
    // ?ô¶êÈ¡¢ÛİêÈ¡¢ä¨êÈ¡¢?êÈ?í®îÜ4êÈBDC?î¤??Î·ß¾?ãÆ
    seg (
        .reset(reset),
        .clock(clock),
        .an(an),
